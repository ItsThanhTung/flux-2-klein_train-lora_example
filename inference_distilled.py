...
if __name__ == "__main__":
    ...
    model_id = getattr(config, "pretrained_model_name_or_path", None)
    # Distilled checkpoint: base-9B → klein-9B, base-4B → klein-4B
    if model_id and "9B" in str(model_id):
        infer_model = "black-forest-labs/FLUX.2-klein-9B"
    elif model_id and "4B" in str(model_id):
        infer_model = "black-forest-labs/FLUX.2-klein-4B"
    else:
        infer_model = "black-forest-labs/FLUX.2-klein-9B"
    pipe = Flux2KleinPipeline.from_pretrained(infer_model, torch_dtype=torch.bfloat16)
    pipe.to("cuda")
