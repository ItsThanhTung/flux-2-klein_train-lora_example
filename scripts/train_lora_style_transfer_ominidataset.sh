...
# Dataset: data/ominidataset/pairs.json (2605 pairs)
# total_batch = 1 * 4 GPUs * 8 accum = 32
accelerate launch \
  --config_file "$ROOT/configs/accelerate_4gpu.yaml" \
  --num_processes 4 \
  --mixed_precision bf16 \
  train_lora_base.py \
  --config configs/train_lora_style_transfer_ominidataset_9b.yaml \
  --output_dir exps/train_lora_style_transfer_ominidataset_9b_r64 \
  2>&1 | tee -a train_logs/train_lora_style_transfer_ominidataset_9b_r64.log
